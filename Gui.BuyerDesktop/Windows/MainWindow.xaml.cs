...
namespace Gui.BuyerDesktop.Windows
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            /// На практике это будет команда, в конструктор которой
            /// будет передаваться вьюмодель. Иначе откуда мы возьмем
            /// данные для printTask (тот же код товара).
            /// PrintTestLabelCommand(this). Мы же этикетки можем
            /// печатать из разных мест приложения, но посредником всегда
            /// является вьюмодель. При этом вьюмодель должна будет реализовать
            /// интерфейс ICanPrint_TestLabel, чтобы команда через this
            /// могла получить нужные поля
